            ...
            <div>
              {this.state.ismessagehiddin ? null : (this.state.content == item.post) ?
                <div className="input-group input-group-lg">
                  <input className="form-control" placeholder="Send a message..." onChange={this.onChange} value={this.state.input} name="input" />
                  <div class="input-group-append">
                    <Button className="btn btn-outline-secondary btn-lg" onClick={() => this.addMessage(this.state.to, this.state.input)}>Send</Button>
                  </div>
                </div> : null}
            </div>
          </div>
